-- 2. delete_child_profile 동시 삭제 경쟁 조건 방지를 위한 advisory lock 추가 및 감사로그 스키마 수정
...
BEGIN
  -- 동시 호출 직렬화를 위한 advisory lock 획득
...
REVOKE EXECUTE ON FUNCTION public.delete_child_profile(UUID, UUID) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.delete_child_profile(UUID, UUID) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.delete_child_profile(UUID, UUID) TO service_role;

-- 자녀가 이미 지워진 상태에서 감사로그가 child_profiles(id)를 외래키로 가리키면 insert 시 에러가 나거나
-- ON DELETE CASCADE로 인해 감사로그도 같이 지워지는 문제가 있으므로 외래키 제약조건 제거
ALTER TABLE public.account_management_audit_log
  DROP CONSTRAINT IF EXISTS account_management_audit_log_child_id_fkey;

-- 실패 사유 기록을 위한 reason 컬럼 추가
ALTER TABLE public.account_management_audit_log
  ADD COLUMN IF NOT EXISTS reason TEXT;

-- action check 제약 조건에 delete_child 및 auth_delete_failed 추가
ALTER TABLE public.account_management_audit_log
  DROP CONSTRAINT IF EXISTS account_management_audit_log_action_check;

ALTER TABLE public.account_management_audit_log
  ADD CONSTRAINT account_management_audit_log_action_check
  CHECK (action IN ('view_account', 'reset_password', 'delete_child', 'auth_delete_failed'));
