...
- **개발 서버 동작 중 빌드 실행 금지**: Next.js 개발 서버(`npm run dev`)가 구동 중인 상태에서 `npm run build`를 실행하면 `.next` 캐시 파일 충돌로 인해 `Internal Server Error`가 유발됩니다. 빌드 검증을 해야 할 경우, 반드시 기존 개발 서버를 종료하고 `.next` 디렉토리를 완전히 삭제한 뒤에 빌드를 실행해야 합니다.
- **빌드 후 개발 서버 재구동**: 빌드 검증을 위해 기존의 개발 서버를 종료한 경우, 검증을 완료하고 나면 사용자가 즉시 확인할 수 있도록 반드시 백그라운드로 HTTPS 개발 서버(`npm run dev:https`)를 다시 구동하여 켜두어야 합니다.
...
8. 빌드 검증 등으로 로컬 개발 서버가 종료되었을 때는, 최종 보고서를 출력하기 직전에 항상 로컬 HTTPS 개발 서버(`npm run dev:https`)를 다시 가동하여 대기 상태로 유지한다.
